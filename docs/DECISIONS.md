...
- **2026-09-12 — Image storage:** Firebase Storage, used for uploaded screenshots and client photos. Anyone can view an image; only a signed-in admin can upload or delete one. Rules enforce a 5MB limit and image-only file types server-side (not just in the app), defined in `storage.rules` — same as `firestore.rules`, only takes effect once published in the Firebase Console.
- **2026-09-12 — Deleting a project or review deletes its images too:** so storage never accumulates orphaned files for content that no longer exists. A pasted external image URL is left untouched (it isn't ours to delete).
