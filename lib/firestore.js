...
export async function getSiteSettings() {
  try {
    ...
    if (!docSnap.exists()) {
      return { success: true, data: { showAdminLoginLink: true } };
    }
    ...
    return {
      ...
      data: {
        showAdminLoginLink:
          data.showAdminLoginLink === undefined
            ? true
            : data.showAdminLoginLink,
      },
    };
  } catch (error) {
    // If settings can't be read for any reason, default to showing the
    // admin login link rather than silently hiding your own way in.
    return { success: true, data: { showAdminLoginLink: true } };
  }
}
