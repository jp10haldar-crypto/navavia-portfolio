...
// getSiteSettings() is called independently by several components on
// every single page (Header, Footer, the page itself, and sometimes
// ClosingCTA too) — without this, one page view meant 3-4 separate,
// identical reads of the exact same tiny document. This cache makes every
// one of those calls within a few seconds of each other share a single
// real database read: if a read is already in flight, later callers just
// wait for that same result instead of starting a new one; once
// settled, the result is reused for a short time after. Cleared
// automatically the moment an admin actually saves new settings, so a
// change is never left showing stale data in the same browser session.
let siteSettingsCache = null; // { data, expiresAt }
let siteSettingsInFlight = null;
const SITE_SETTINGS_CACHE_MS = 10000;

function normalizeSiteSettings(saved) {
  return {
...
export async function getSiteSettings() {
  if (siteSettingsCache && Date.now() < siteSettingsCache.expiresAt) {
    return { success: true, data: siteSettingsCache.data };
  }
  if (siteSettingsInFlight) {
    return siteSettingsInFlight;
  }

  siteSettingsInFlight = (async () => {
    try {
      const docSnap = await runWithTimeout(() =>
        getDoc(doc(db, SETTINGS_COLLECTION, SITE_SETTINGS_DOC_ID))
      );
      const data = docSnap.exists()
        ? normalizeSiteSettings(docSnap.data())
        : DEFAULT_SITE_SETTINGS;
      siteSettingsCache = { data, expiresAt: Date.now() + SITE_SETTINGS_CACHE_MS };
      ...
      // Not cached, so the next call tries the real database again.
...
export async function updateSiteSettings(settingsData) {
  try {
    ...
    // Clears the short cache above, so anything reading settings right
    // after a save (elsewhere in the same browser session) sees the
    // change immediately instead of a stale cached copy.
