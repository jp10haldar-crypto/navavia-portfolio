...
// directly — it
// always goes through one of these functions instead. Every function
// returns the same simple shape: { success: true, data } when it worked,
// or { success: false, message } with a plain-English explanation when it
// didn't — so the screen that called it can always show something
...
export async function getHomepageVideoById(id) {
  try {
    const docSnap = await getDoc(doc(db, HOMEPAGE_VIDEOS_COLLECTION, id));
    if (!docSnap.exists()) {
...
export async function updateHomepageVideo(id, videoData) {
  try {
    await updateDoc(doc(db, HOMEPAGE_VIDEOS_COLLECTION, id), videoData);
    return { success: true };
  } catch (error) {
    return {
      success: false,
...
export async function deleteHomepageVideo(id) {
  try {
    await deleteDoc(doc(db, HOMEPAGE_VIDEOS_COLLECTION, id));
    return { success: true };
  } catch (error) {
    return {
      success: false,
...
export async function seedHomepageVideos() {
  ...
  try {
    for (const video of starterHomepageVideos) {
      ...
      await setDoc(
        doc(db, HOMEPAGE_VIDEOS_COLLECTION, String(id)),
        videoData
      );
    }
  } catch (error) {
...
// Gets one blog post by its web-address slug. Used by the public post
// page (only succeeds for a draft if the visitor happens to be the
// signed-in admin — see firestore.rules) and by the admin form to check
...
export async function getBlogPostById(id) {
  try {
    const docSnap = await getDoc(doc(db, BLOG_POSTS_COLLECTION, id));
    if (!docSnap.exists()) {
      return { success: false, message: "That post could not be found." };
    }
    return { success: true, data: { id: docSnap.id, ...docSnap.data() } };
...
export async function updateBlogPost(id, postData) {
  try {
    await updateDoc(doc(db, BLOG_POSTS_COLLECTION, id), postData);
    return { success: true };
  } catch (error) {
    return {
      success: false,
